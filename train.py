...
def get_parser():  # 在Python中提供了argparse模块，用于处理命令行选项、参数和子命令解析器等相关内容。此模块可以让人快速编写用户友好的命令行接口内容。
    parser = argparse.ArgumentParser(description='PyTorch Few-Shot Semantic Segmentation')
    parser.add_argument('--arch', type=str, default='SAM_RSP')
    parser.add_argument('--viz', action='store_true', default=False)
    parser.add_argument('--config', type=str, default='/home/data/ljg/zw/SAM-RSP-main/config/pascal/coco_split0_vgg.yaml',
                        help='config file')  # 只是默认设置，实际在.sh文件中修改
    parser.add_argument('--local_rank', type=int, default=-1,
                        help='number of cpu threads to use during batch generation')
    parser.add_argument('--opts', help='see config/ade20k/ade20k_pspnet50.yaml for all options',
                        default=None, nargs=argparse.REMAINDER)
    parser.add_argument('--exp_path', type=str, default='/home/data/ljg/zw/SAM-RSP-main/exp/Train/pascal/SAM_RSP/split0/vgg')
    parser.add_argument('--snapshot_path',type=str,default='/home/data/ljg/zw/SAM-RSP-main/exp/Train/pascal/SAM_RSP/split0/vgg/snapshot')
    parser.add_argument('--result_path',type=str, default='/home/data/ljg/zw/SAM-RSP-main/exp/Train/pascal/SAM_RSP/split0/vgg/result')
    parser.add_argument('--show_path',type=str, default='/home/data/ljg/zw/SAM-RSP-main/exp/Train/pascal/SAM_RSP/split0/vgg/show')
...
def main():
    ...
    args = get_parser()  # 获取解析器实例，编写命令行参数接口
    logger = get_logger()  # 获取记录器实例
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch % args.save_freq == 0) and (epoch > 0) and main_process():
            # 每save_freq次记录一次断点，如果从断点继续运行需要更改resume路径
        ...
        if args.evaluate and epoch % 1 == 0:
            loss_val, mIoU_val, mAcc_val, allAcc_val, class_miou_b, class_miou = validate(val_loader, model)
            val_num += 1
            if main_process() and args.viz:
            ...
            if class_miou > max_iou:
                ...
                if args.shot == 1:
                    filename = args.snapshot_path + '/train_epoch_' + str(epoch) + 'miou_b_{:.4f}'.format(class_miou_b) + 'miou_{:.4f}'.format(max_iou) + '.pth'
                else:
                    filename = args.snapshot_path + '/train{}_epoch_'.format(args.shot) + str(epoch) + 'miou_b_{:.4f}'.format(class_miou_b) + 'miou_{:.4f}'.format(max_iou) + '.pth'
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        output, aux_loss, main_loss = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b, cat_idx=subcls)
        loss = args.loss_weight[0]*aux_loss + args.loss_weight[1] * main_loss
        ...
        intersection, union, target = intersectionAndUnionGPU(output, target, args.classes, args.ignore_label)  # 返回的分别是记录每个类别的像素个数的一维数组
        # output 网络输出 target查询集真实mask    args.classes=21（ymal中可以配置） args.ignore_label=255（ymal中可以配置）
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'Remain {remain_time} '
                        'Main_loss {main_loss_meter.val:.4f} '
                        'Aux_loss {aux_loss_meter.val:.4f} '
        ...
        if args.evaluate and args.SubEpoch_val and (args.epochs <= 100 and epoch % 1 == 0 and epoch > 0) and (
            ...
            if class_miou > max_iou:
                ...
                if args.shot == 1:
                    filename = args.snapshot_path + '/train_epoch_' + str(epoch - 0.5) + 'miou_b_{:.4f}'.format(class_miou_b) + 'miou_{:.4f}'.format(max_iou) + '.pth'
                else:
                    filename = args.snapshot_path + '/train{}_epoch_'.format(args.shot) + str(epoch - 0.5) + 'miou_b_{:.4f}'.format(class_miou_b) + 'miou_{:.4f}'.format(max_iou) + '.pth'
                if main_process():
                    logger.info('Saving checkpoint to: ' + filename)
                    torch.save(
    ...
    iou_class = intersection_meter.sum / (union_meter.sum + 1e-10)  # 返回的是每个类别对应的iou的一维数组 [class1:0.7.....classk: 0.8]
    accuracy_class = intersection_meter.sum / (target_meter.sum + 1e-10)  # 与上一个同理
...
def validate(val_loader, model):
    ...
    intersection_meter_b = AverageMeter()
    ...
    return loss_meter.avg, mIoU, mAcc, allAcc, class_miou_b, class_miou,
