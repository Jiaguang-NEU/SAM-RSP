...
def intersectionAndUnionGPU(output, target, K, ignore_index=255):
    ...
    intersection = output[output == target]  #交集
...
def get_save_path(args):
    ...
    args.snapshot_path = 'exp/{}/{}/split{}/{}/snapshot'.format(args.data_set, args.arch, args.split, backbone_str)#相对路径
...
def memobank(supp_pro_list, supp_pro_mean, lenth, cls, memory_bank, ptr):
    supp_pro = torch.tensor(torch.cat(supp_pro_list, 2))
    for i in range(supp_pro.shape[0]):
        if (ptr[cls[0][i]]+supp_pro.shape[2])<lenth:
            memory_bank[cls[0][i], int(ptr[cls[0][i]]):int(ptr[cls[0][i]])+supp_pro.shape[2],:,:,:] = supp_pro[i,:,:,:,:].permute(1,0,2,3)
            ptr[cls[0][i]] += supp_pro.shape[2]
        else:
            memory_bank[cls[0][i], int(ptr[cls[0][i]]):lenth,:,:,:] = supp_pro[i,:,:,:,:].permute(1,0,2,3)[:(lenth-int(ptr[cls[0][i]])),:,:,:]
            memory_bank[cls[0][i], :(supp_pro.shape[2]-(lenth-int(ptr[cls[0][i]]))),:,:,:] = \
                supp_pro[i,:,:,:,:].permute(1, 0, 2, 3)[(lenth - int(ptr[cls[0][i]])):, :, :, :]
            ptr[cls[0][i]] = supp_pro.shape[2]-(lenth-int(ptr[cls[0][i]]))
    mean_pro = torch.mean(memory_bank,dim=1)
    loss_memo = 0
    for i in range(supp_pro.shape[0]):
        a = supp_pro_mean[i].view(1,-1)
        b = mean_pro[cls[0][i]].view(1,-1)
        loss_memo += torch.cosine_similarity(a, b)
    loss_memo = -loss_memo / supp_pro.shape[0] + 1
