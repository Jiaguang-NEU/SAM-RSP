...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None,
        ...
        self.base_path = '/home/data/ljg/zw/SAM-RSP-main' + os.path.join(self.base_data_root, mode, str(self.split)).split('..')[1]
        ...
        with open(fss_sub_class_file_list_path, 'r') as f:
            f_str = f.read()
        self.sub_class_file_list = eval(f_str)  # sub_class_file_list 的路径有分组 与 fss_data_list不一样
    ...
    def __getitem__(self, index):
        ...
        image_path = '/home/data/ljg/zw/SAM-RSP-main' + image_path.split('..')[1]
        label_path = '/home/data/ljg/zw/SAM-RSP-main' + label_path.split('..')[1]
        ...
        for k in range(self.shot):
            ...
            support_image_path = '/home/data/ljg/zw/SAM-RSP-main' + support_image_path.split('..')[1]
            support_label_path = '/home/data/ljg/zw/SAM-RSP-main' + support_label_path.split('..')[1]
...
class GSemData(Dataset):
    # Generalized Few-Shot Segmentation    广义少镜头分割
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None,
                 use_split_coco=False, \
...
class BaseData(Dataset):
    ...
    def __getitem__(self, index):
        image_path, label_path = self.data_list[index]
        image = cv2.imread(image_path, cv2.IMREAD_COLOR)  # 读取图片和label
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image = np.float32(image)
        label = cv2.imread(label_path, cv2.IMREAD_GRAYSCALE)
